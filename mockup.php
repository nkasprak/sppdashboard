                                ...
                                foreach ($statesArr as $sid=>$state) {
                                    ...
                                    foreach ($columnsArr as $cid=>$column) {
										...
										if ($column['tabAssoc'] == $tabIndex) {
											...
											<span>*/
											if ($column["mode"] == "date") {
												if (!empty($data["display_data"]) && empty($data["sort_data"])) {
													$timestamp = ($data["display_data"])*86400;
													$theDate = date("M j",$timestamp);
													$data["sort_data"] = $data["display_data"];
											...
											if ($column["mode"] == "numeric" && is_numeric($data["display_data"])) {
												if (is_numeric($column["roundTo"])) $data["display_data"] = round($data["display_data"],$column["roundTo"]);
												if (!empty($column["prepend"])) {
													$data["sort_data"] = $data["display_data"];
													if ($data["display_data"] < 0) {
